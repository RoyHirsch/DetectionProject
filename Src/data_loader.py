...
class BusDataLoader(Dataset):
    def __init__(self, root_dir, data_loader_type='', BGpad=16, outShape=225, balance_data_size=2, augment_pos=0):
        ...
        __getitem__ - generates the relevant data of single example:
                      ...
                      cls_label    - GT classification label (binary)
                      rg_label     - GT regression label, (list), the GT bounding box as [x, y, h, w, class]
        ...
        value_count = self.rect_data['cls_label'].value_counts().to_dict()
    ...
    def _augment_pos(self):
        ...
        '''
        columns_list_rois = ['image_name', 'rect', 'cls_label', 'reg_label']
        permutation_number = self.augment_pos
        pos_rows = self.rect_data.loc[self.rect_data['cls_label'] == 1]
        ...
        for row in pos_rows.iterrows():
            ...
            reg_label = list(row[1]['reg_label'])
            ...
            for num in range(permutation_number):
                ...
                perm_list.append([im_name, [int(xt), int(yt), int(wt), int(ht)], 1, reg_label])
    ...
    def _balance_data(self):
        ...
        self.rect_data = self.rect_data.sort_values(by=['cls_label'], ascending=False)
        self.rect_data.drop(self.rect_data.tail(num_of_drop_rows).index, inplace=True)

        # Shuffle the dataFrame
        self.rect_data = self.rect_data.sample(frac=1).reset_index(drop=True)

    def _process_data(self, columns_list_rois=['image_name', 'rect', 'cls_label', 'reg_label']):
        ...
        for root, dirs, files in os.walk(os.path.join(self.root_dir, 'rects')):
            for name in files:
                if re.findall(r'rois_for_.*', name):
                    ...
                    temp = pd.DataFrame([[split_name[2], rect[:4], int(rect[4:5]), rect[5:]] for rect in rects], columns=columns_list_rois)
    ...
    def __getitem__(self, idx):
        ...
        name, rect, cls_label, rg_label = self.rect_data.loc[idx]
        ...
        # sampel.sape = [C, H, W]
        return sample, torch.tensor(cls_label).long(), torch.tensor(rg_label).long(), torch.tensor(rect).float()
...
img, cls_label, rg_label, rect = TrainDataLoader.__getitem__(2814)
img, cls_label, rg_label, rect = TrainDataLoader.__getitem__(650)
'''
